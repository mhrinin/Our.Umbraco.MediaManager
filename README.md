...
```bash
# Umbraco 16 / 17
dotnet add package Our.Umbraco.MediaManager
...
```bash
# Frontend (Lit + Vite)
cd src/Our.Umbraco.MediaManager/Client
npm ci
npm run build          # emits to ../wwwroot/App_Plugins/MediaManager

# Run the sample hosts (SQLite, unattended install)
cd ../../../samples/Our.Umbraco.MediaManager.Web        # Umbraco 17
dotnet run
cd ../Our.Umbraco.MediaManager.Web.V16                  # Umbraco 16
dotnet run
```
...
```
src/Our.Umbraco.MediaManager      # the package (C# backend + Client/ frontend)
...
`Our.Umbraco.MediaManager`, Workflow File `release.yml`) and set the `NUGET_USER` repository
